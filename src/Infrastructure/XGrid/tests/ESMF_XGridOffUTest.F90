...
!==============================================================================
!BOP
! !PROGRAM: ESMF_XGridOffUTest - Unit tests for Field Create and Get methods
!
! !DESCRIPTION:
!
...
!------------------------------------------------------------------------------
  ...
  type(ESMF_Grid) :: gridA, gridB, gridA1D(1), gridA2D(1)
  type(ESMF_XGrid) :: xgrid, xgridout
  type(ESMF_VM) :: vm
  ...
  gridA = ESMF_GridCreateMosaic(filename=gridAfile, &
...
!  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
  ...
  isCreated =ESMF_GridIsCreated(gridA, rc=localrc)
  call ESMF_TEST((localrc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
  call ESMF_GridValidate(gridA, rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE

  call ESMF_GridWriteVTK(GridA, filename='AltTst_GridA', rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE

  gridBfile = 'data/ocean_mosaic.nc'
  print *, "Reading OCN mosaic to grid, ", gridBfile
  gridB = ESMF_GridCreateMosaic(filename=gridBfile, &
                staggerLocList= staggerLocList, &
                coordTypeKind = ESMF_TYPEKIND_R8, &
                tileFilePath='./data/', rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
  ...
  write(failMsg, *) "Did not return ESMF_SUCCESS"
  isCreated =ESMF_GridIsCreated(gridB, rc=localrc)
  call ESMF_TEST((localrc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
  call ESMF_GridValidate(gridB, rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE

  call ESMF_GridWriteVTK(gridB, filename='AltTst_GridB', rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
  ...
  distgrid1D = ESMF_DistGridCreate(minIndex=(/1/), maxIndex=(/99999/), &
	regDecomp=(/petCount/), rc=localrc)
  distgrid2D = ESMF_DistGridCreate(minIndex=(/1,1/), maxIndex=(/99999,99999/), &
        regDecomp=(/petCount,1/), rc=localrc)

   call ESMF_GridAddItem(gridB,staggerLoc=ESMF_STAGGERLOC_CORNER, &
      itemflag=ESMF_GRIDITEM_MASK, rc=localrc)
   if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE

  ! Get isPresent, for item that is present
  call ESMF_GridGetItem(gridB, staggerloc=ESMF_STAGGERLOC_CORNER, &
  ...
  call ESMF_GridWriteVTK(gridB, filename='AltTst_GridB_mask', rc=localrc)
  if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
...
!     fieldRd(1)=ESMF_FieldCreate(xgrid=xgrid, TYPEKIND=ESMF_TYPEKIND_R8,  &
!        ungriddedLBound=(/1/), ungriddedUBound=(/2/), gridToFieldMap=(/2/), &
!       indexflag=ESMF_INDEX_DELOCAL, name="tile1_cell",  rc=localrc)
!     fieldRd(2)=ESMF_FieldCreate(xgrid=xgrid, TYPEKIND=ESMF_TYPEKIND_R8,  &
!       indexflag=ESMF_INDEX_DELOCAL, name="tile2_cell",  rc=localrc)
!     fieldRd(3)=ESMF_FieldCreate(xgrid=xgrid, TYPEKIND=ESMF_TYPEKIND_R8,  &
!       indexflag=ESMF_INDEX_DELOCAL, name="xgrid_area",  rc=localrc)
...
!     print *, "Build offline xgrid"
!     call build_offline_xgrid(xgrid, state, xgridout, rc)
!     if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE  
...
!     if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
     ...
     !compute flux
 !     print *, "Validating 2nd order flux conservation on offline xgrid"
 !     call flux_exchange_sph(xgridout, rc=localrc)
 !     if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE  
...
#undef  ESMF_METHOD
#define ESMF_METHOD "build_offline_xgrid"
    subroutine build_offline_xgrid(xgrid, xgridfile, xgridout, rc)
          !----------------------------------------------------
          ! build_offline_xgrid
          ! This method generates an ESMF-formatted xgrid from
          ! a reference ESMF xgrid and a FMS offline xgrid that
          ! has been already read to an ESMF state
          !----------------------------------------------------
          integer, intent(out) :: rc
          type(ESMF_XGrid), intent(inout) :: xgrid
          type(ESMF_XGrid), intent(out) :: xgridout
          character(128), intent(in) :: XgridFile

          type(ESMF_Grid),allocatable :: gridA(:), gridB(:)
          type(ESMF_Field) :: f_atm, f_ocn, field
          ...
          if (ESMF_LogFoundError(localrc, &
              ...
              ! Define specs and initialize arrays for writing
              call ESMF_ArraySpecSet(arrayspec1D, rank=1, typekind=ESMF_TYPEKIND_R8, rc=localrc)
              call ESMF_ArraySpecSet(arrayspec2D, rank=2, typekind=ESMF_TYPEKIND_R4, rc=localrc)
              distgrid1D = ESMF_DistGridCreate(minIndex=(/1/), maxIndex=(/99999/), &
                    regDecomp=(/petCount/), rc=localrc)
              distgrid2D = ESMF_DistGridCreate(minIndex=(/1,1/), maxIndex=(/99999,99999/), &
                    regDecomp=(/petCount,1/), rc=localrc)
            ...
            call ESMF_XGridGet(xgrid, mesh=xgrid_mesh, rc=localrc)
             ...
             print *, "add fields to fieldbundle"
              ...
              allocate(flds(3))
               ...
               do n = 1,size(flds)
                 fieldRd = ESMF_FieldCreate(xgrid_mesh, ESMF_TYPEKIND_R8, &
                         name=trim(flds(n)), meshloc=ESMF_MESHLOC_ELEMENT, rc=localrc)
                 call ESMF_FieldGet(fieldRd, farrayptr=ptr, rc=localrc)
                  ptr(:) = 0.0
                  nullify(ptr)
                  call ESMF_FieldBundleAdd(FBin, (/fieldRd/), rc=localrc)
               ...
               if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE
                ...
                call ESMF_FieldBundleGet(FBin, fieldCount=fieldCount, rc=localrc)
                if (localrc .ne. ESMF_SUCCESS) rc=ESMF_FAILURE

                do i = 1, fieldCount
                  call ESMF_FieldBundleGet(FBin, (/flds(i)/), field=field, rc=localrc)
    ...
    end subroutine build_offline_xgrid

end program ESMF_XGridOffUTest
