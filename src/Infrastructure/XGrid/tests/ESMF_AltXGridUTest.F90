...
!==============================================================================
!BOPI
! !PROGRAM: ESMF_AltXGridUTest - Unit tests for Field Create and Get methods
!
! !DESCRIPTION:
!
...
!------------------------------------------------------------------------
  ...
  subroutine test7(rc)
    integer, intent(out)                :: rc
    integer                             :: localrc, i, npet, tile
    real(ESMF_KIND_R8)			:: threshhold
    type(ESMF_XGrid)                    :: xgrid
    character	                        :: AtmFile, OcnFile
    type(ESMF_Grid)			:: gridA(1), gridB(1)
    type(ESMF_Grid)                     :: sideA(2), sideB(1)
    type(ESMF_Grid)                     :: AtmGrid, OcnGrid
    ...
    type(ESMF_Field)                    :: xfieldin
    type(ESMF_Grid)                     :: xgridin, dirxgridin
...
!    print *, "Read ATM mosaic to grid"
!    AtmFile = "/work/noaa/epic-ps/bweir/esmf/src/Infrastructure/XGrid/tests/data/C48_mosaic.nc"
!    call ESMF_GridSpecReadMosaic(AtmFile, &
!               mosaic, tileFilePath='./data/', rc=localrc)
!    print *, mosaic%ntiles, "/", mosaic%nx, "/",  mosaic%ny
!    print *, trim(mosaic%tileDirectory)//trim(mosaic%filenames(tile))
!   gridA(1) = ESMF_GridCreateMosaic(filename='data/C48_mosaic.nc', &
...
!    call ESMF_Test((rc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
!    call ESMF_GridValidate(grid(A), rc=localrc)
!    if (ESMF_LogFoundError(localrc, &
!      ESMF_ERR_PASSTHRU, &
!      ESMF_CONTEXT, rcToReturn=rc)) return


!    print *, "Read OCN mosaic to grid"
!
!    OcnFile = './data/ocean_mosaic.nc'
!    tile = 1
!    call ESMF_GridSpecReadMosaic(OcnFile, &
!               mosaic, tileFilePath='./data/', rc=localrc)
!    print *, mosaic%ntiles, "/", mosaic%nx, "/",  mosaic%ny
!    print *, trim(mosaic%tileDirectory)//trim(mosaic%filenames(tile))
!     gridB(1) = ESMF_GridCreateMosaic(filename='data/ocean_mosaic.nc', &
!                staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER/), &
!                coordTypeKind = ESMF_TYPEKIND_R8, &
!                tileFilePath='./data/', rc=localrc)
!    if (ESMF_LogFoundError(localrc, &
...
!    write(failMsg, *) "Did not return ESMF_SUCCESS"
!    isCreated = ESMF_GridIsCreated(OcnGrid, rc=localrc)
!    call ESMF_Test((rc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
...
!    sideA(1) = make_grid_sph(4,4,1.,1.,0.,0.,area_adj=0.95, rc=localrc)
!    sideA(2) = make_grid_sph(4,4,0.6,1.,3.5,3.5,area_adj=0.95, rc=localrc)
!    sideB(1) = make_grid_sph(8,8,1.,1.,0.,0.,area_adj=0.95, rc=localrc)
    ...
    !  storeOverlay = .true., &
    !  rc=localrc)
!     xgrid = ESMF_XGridCreate(sideAGrid=(/sideA(1),sideA(2)/), &
!                               sideBGrid=(/sideB(1)/), &
!      storeOverlay = .true., &
!      rc=localrc)
    if (ESMF_LogFoundError(localrc, &
...
!    print *, "Exporting XGrid NETCDF"
!    f_xgrid = ESMF_FieldCreate(xgrid=xgrid, TYPEKIND=ESMF_TYPEKIND_R8, rc=localrc)
!    call ESMF_FieldWrite(f_xgrid, fileName="AltXgridOutTest.nc", &
  ...
  subroutine flux_exchange_sph(xgrid, scheme, area_adj, rc)
    ...
    do i = 1, size(srcField)
      ...
      if (ESMF_LogFoundError(localrc, &
          ESMF_ERR_PASSTHRU, &
          ESMF_CONTEXT, rcToReturn=rc)) return
   !   call ESMF_GridWriteVTK(srcGrid(i), cids(i)//'_srcgrid.vtk', rc=localrc)
   !   if (ESMF_LogFoundError(localrc, &
   !       ESMF_ERR_PASSTHRU, &
   !       ESMF_CONTEXT, rcToReturn=rc)) return
    enddo
    do i = 1, size(dstField)
      ...
      if (ESMF_LogFoundError(localrc, &
          ESMF_ERR_PASSTHRU, &
          ESMF_CONTEXT, rcToReturn=rc)) return
  !    call ESMF_GridWriteVTK(dstGrid(i), cids(i)//'_dstgrid.vtk', rc=localrc)
  !    if (ESMF_LogFoundError(localrc, &
  !        ESMF_ERR_PASSTHRU, &
  !        ESMF_CONTEXT, rcToReturn=rc)) return
    enddo
    ...
    ! make sure flux is conserved on XGrid
 !   call ESMF_XGridWriteVTK(xgrid, 'xgrid.vtk', rc=localrc)
 !   if (ESMF_LogFoundError(localrc, &
 !       ESMF_ERR_PASSTHRU, &
 !       ESMF_CONTEXT, rcToReturn=rc)) return
    allocate(exf_area(lbound(exf,1):ubound(exf,1)))
